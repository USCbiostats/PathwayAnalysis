...
LisaNetForNodes <- function(ntwk){
  V(ntwk)$StandardizedAttribute <- rep(-9,length(V(ntwk)))
  AttributeMean <- mean(V(ntwk)$MyAttribute)
  # Standardize node labels - subtract the mean attrubute value from all attributes
  V(ntwk)$StandardizedAttribute <- V(ntwk)$MyAttribute - AttributeMean
  L <- rep(0,length(V(ntwk)))
  #NeighborsLists <- replicate(length(V(ntwk)), c(), simplify = FALSE)
  for(i in 1:length(E(ntwk))){
    a=as.integer(get.edgelist(ntwk)[i,1])
    b=as.integer(get.edgelist(ntwk)[i,2])
    addvalue=V(ntwk)$StandardizedAttribute[a] * V(ntwk)$StandardizedAttribute[b]
    L[a]=L[a]+addvalue
    L[b]=L[b]+addvalue
  }
  return (L)
...
PValue=mclapply(1:SizeOfOurNetwork, f)

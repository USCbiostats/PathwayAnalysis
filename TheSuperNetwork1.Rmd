...
SpatiallyCorrelatedLabels3 <- function(ntwk,CentralNode,NeighborhoodDistance){
  ...
  cat("\nThere are ",NbrCount, " nodes within  distance of ",NeighborhoodDistance)
  return (ntwk)
}
...
The permutation test is now written as a function.
...
set.seed(45642)  # Error when seed is 45644 - fixed
NPerms <- 250
...
WhichIsFocal <- which(V(NewSub)$MyID==FocalNode)


# Generate some attribute values
newnet <- SpatiallyCorrelatedLabels3(NewSub,FocalNode,1)
NewSub <- CopyNet2(newnet) 
...
OldLisaNetForNodeI <- function(ntwk,IthNode){
  V(ntwk)$StandardizedAttribute <- rep(-9,length(V(ntwk)))
  AttributeMean <- mean(V(ntwk)$MyAttribute)
  # Standardize node labels - subtract the mean attribute value from all attributes
  V(ntwk)$StandardizedAttribute <- V(ntwk)$MyAttribute - AttributeMean
  for (i in IthNode:IthNode){  # This loop can be removed. It's a legacy of code I borrowed from a Moran's-I function
    L <- 0
    for (j in 1:length(V(ntwk))){
      if (distances(ntwk,v=V(ntwk)[i],to=V(ntwk)[j]) == 1){
        # they are neighbors - for now we only consider directly adjacent nodes
...
plot(NewSub, edge.arrow.size=.2,  vertex.size= as.numeric(ifelse(V(NewSub)$MyAttribute < 0,"0.1",2+V(NewSub)$MyAttribute)),main="original attributes",vertex.label.cex=0.2)

# OBserved LStat value:
ObservedLISA <- OldLisaNetForNodeI(NewSub,WhichIsFocal)

# Function to permute the attribute values and recalculate Local Moran's I
#PValue <- rep(0,SizeOfOurNetwork)
LStat <- rep(0,NPerms)

PermuteAndRecalculate <- function(ntwk,Focus){
  WhichIsFocal <- which(V(ntwk)$MyID==Focus)
  V(ntwk)$PermedAttribute <- FixedPerm(V(ntwk)$MyAttribute,WhichIsFocal)
  V(ntwk)$MyAttribute <- V(ntwk)$PermedAttribute

  #plot(ntwk, edge.arrow.size=.2,  vertex.label.cex=0.2, vertex.size= as.numeric(ifelse(V(ntwk)$PermedAttribute < 0,"0.1",2+V(ntwk)$PermedAttribute)),main="attributes of sample permuted network")
  
  LMI <- OldLisaNetForNodeI(ntwk,WhichIsFocal)
  #cat("\nLMI:",LMI)
  return(LMI)
}

#(head(V(net2)$MyAttribute))
for (kk in 1:NPerms){
  LStat[kk] <- PermuteAndRecalculate(NewSub,FocalNode)
}
#(head(V(NewSub)$MyAttribute))
PValue <- sum(LStat>ObservedLISA)/NPerms
cat("\nNewPValue: ",PValue)
boxplot(LStat)
abline(h=ObservedLISA,col="red",main="New distribution of LISA Stat")
cat("\nLISA stat value for focal node: ",ObservedLISA)
NewLStat <- LStat



# The following is old code to check we still get the same answer now that we have re0written thingds...

# permute the network and re-evaluate
net2 <- CopyNet2(NewSub)
i <- FocalNode
...
for (i in WhichIsFocal:WhichIsFocal)
{
  ...
  LStat=unlist(LStat)
  PValue[WhichIsFocal] <- sum(LStat>ObservedLISA)/NPerms
}
...
cat("\nKS test result:\n")
(ks.test(NewLStat,LStat))
ks <- ks.test(NewLStat,LStat)
plot(ecdf(LStat), 
     xlim = range(c(LStat, NewLStat)), 
     col = "blue",pch='.')
plot(ecdf(NewLStat), 
     add = TRUE, 
     lty = "dashed",
...
Now let's try to do repeated permutation tests and look at the distribution of the resulting p-values....
```{r FullPerm}
...
NPerms <- 100 #250
NRepeats <- 20
PVals <- rep(-9,NRepeats)
OrigLISAs <- rep(-9,NRepeats)
nNeighbors <- rep(0,NRepeats)
SubGraphSize <- 500 ## 500

SpatiallyCorrelatedLabels4 <- function(ntwk,CentralNode,NeighborhoodDistance,FocalMean,RemainingMean){
  # label CentralNode and all its near neighbors Normal(FocalMean,1), and everything else Normal(RemainingMean,1).
  ...
  NbrCount<- -1  # Don't want to count the Focal Node itself
  for (i in 1:length(V(ntwk))){
    if ((distances(ntwk,CenterNode,i) <= NeighborhoodDistance)&&(NbrCount<50000)){
      V(ntwk)$MyAttribute[i] <- rnorm(1,FocalMean,1)
      NbrCount <- NbrCount +1
    }
    else
      V(ntwk)$MyAttribute[i] <- rnorm(1,RemainingMean,1)
  }
  cat("\nThere are ",NbrCount, " nodes within  distance of ",NeighborhoodDistance)
  nNeighbors[mm] <<-NbrCount
  return (ntwk)
}

foreach (mm=1:NRepeats) %do% {
  ...
  # Generate some attribute values
  newnet <- SpatiallyCorrelatedLabels4(NewSub,FocalNode,2,3,2)
  NewSub <- CopyNet2(newnet) 
 
  #plot(NewSub, edge.arrow.size=.2,  vertex.size= as.numeric(ifelse(V(NewSub)$MyAttribute < 0,"0.1",2+V(NewSub)$MyAttribute)),main=c("original atts. Iter: ",mm),vertex.label.cex=0.2)

  ObservedLISA <- OldLisaNetForNodeI(NewSub,WhichIsFocal)
  OrigLISAs[mm] <- ObservedLISA
  LStat <- rep(0,NPerms)
  ...
  #(head(V(NewSub)$MyAttribute))
  PVals[mm] <- sum(LStat>ObservedLISA)/NPerms
  cat("\nNewPValue: ",PVals[mm],"   orig. LisaStat: ",ObservedLISA)
  if (1){ #(PVals[mm]>0){
      plot(NewSub, edge.arrow.size=.2,  vertex.size= as.numeric(ifelse(V(NewSub)$MyAttribute < 2,"2",2+3*V(NewSub)$MyAttribute)),main=c("original atts. Iter: ",mm," p=",PVals[mm]),vertex.label.cex=0.2)
...
(boxplot(PVals))
plot(nNeighbors,PVals)


```

Let's look at how the subgraphs resemble the overall super network in terms of various summary statistics:
```{r,   NetworkSummaries}
y <- degree_distribution(FullNetwork)[1:150]
x <- 1:length(y)
lo <- loess(y~x)
plot(x,y,pch='.',main="Supernetwork Degree Distn (truncated at 150")
...
for (ii in 1:5){
  FocalNode <- sample(1:vcount(FullNetwork), 1)  # this was how the old function did it
  NewSub <- BuildFocalSubGraph(FullNetwork,SubGraphSize,FocalNode)
  y <- degree_distribution(NewSub)
  x <- 1:length(y)
  lo <- loess(y~x)
  plot(x,y,pch='.',main="Example Subgraph Degree Distn")
  lines(predict(lo), col='red', lwd=2)
}

# add other graph diagnostics
cat("\nDiam. of S: ",diameter(FullNetwork))
cat("\nDiam.of e.g. subgraph: ",diameter(NewSub))
cat("\n\nAv. Path Len of S: ",average.path.length(FullNetwork))
cat("\nAv. Path Len of e.g. subgraph: ",average.path.length(NewSub))

# q: how best to compare two graphs in terms of their overall structure?
# a: use the graph edit distance, which is the minimum number of edge additions, deletions, and substitutions needed to transform one graph into the other.

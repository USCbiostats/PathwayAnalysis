...
# A function to generate attributes and run a permutation test
# First a function to help parallelize
ff <- function(j){
...
# Now the perm test
PermTestForLocalMoransI <- function(SubGraph)
{
  ...
  plot(net2, edge.arrow.size=0.2,  vertex.size= as.numeric(ifelse(V(newnet)$MyAttribute < 0,"0.1",2+V(newnet)$MyAttribute)),main="original attributes",vertex.label.cex=0.02)
  ...
  # permute the network and re-evaluate
  i <- FocalNode
  # We will use a different focal node each time
  WhichIsFocal <- which(V(net2)$MyID==FocalNode)
  #WhichIsFocal <- sample(1:length(V(net2)$MyID),1)
  V(net2)$PermedAttribute <- FixedPerm(V(net2)$MyAttribute,WhichIsFocal)

  plot(net2, edge.arrow.size=.2,  vertex.label.cex=0.02, vertex.size= as.numeric(ifelse(V(net2)$PermedAttribute < 0,"0.1",2+V(net2)$PermedAttribute)),main="attributes of sample permuted network")
  ...
  boxplot(LStat,main=c("Observed LISA stat:",ObservedLISA))
  abline(h=ObservedLISA,col="red",main="Distribution of LISA Stat")
  cat("\nLISA stat value for focal node: ",ObservedLISA,",  focal node is ",WhichIsFocal)
}

for (j in 1:5){
  PermTestForLocalMoransI(NewSub)
}
...
for (kk in 1:NPerms){
  LStat[kk] <- PermuteAndRecalculate(net2,FocalNode)
}
#(head(V(net2)$MyAttribute))
PValue <- sum(LStat>ObservedLISA)/NPerms
cat("nNewPValue: ",PValue)
boxplot(LStat)

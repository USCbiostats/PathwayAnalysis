...
more like single gene pathways.
...
We include all edges from the original graph that connect any two vertices in our subgraph.
...
Here's a smaller network to practice with. It comes from Bryan Queme. It's based on a single pathway and is stored as an igraph called "alz"
```{r, SmallerNetwork}
...
Here we seem to see a reasonable amount of difference.
In thisd example, the focal node only has one neighbor. 

...
SpatiallyCorrelatedLabels <- function(ntwk,CentralNode){
  ...
  V(ntwk)$MyAttribute[CentralNode] <- 5
  for (i in 1:length(V(ntwk))){
...
```{r   OldFixedPerm, echo=FALSE, eval=FALSE}
...
# NEed a new version of this function that passes the focl node as a parameter
...
newnet <- SpatiallyCorrelatedLabels(NewSub,FocalNode)
plot(newnet, vertex.color=ifelse(V(newnet)==FocalNode,"blue","red"), vertex.label.dist=2.5,edge.arrow.size=0.3,vertex.label.cex=ifelse(V(newnet)==FocalNode,1,0.4),vertex.size=ifelse(V(newnet)==FocalNode,10,5))
  
cat("\nMorans-I: ",MoransI(distances(newnet,v=V(newnet),to=V(newnet)),V(newnet)$MyAttribute),"    expectation= ",-1/(length(V(newnet)$MyAttribute)-1))


# Generate some attribute values - use a fixed value for the focal node and it's near neighbors; Unif(0,1) everywhere else
SpatiallyCorrelatedLabels3 <- function(ntwk,CentralNode,NeighborhoodDistance){
  ...
  V(ntwk)$MyAttribute[CentralNode] <- 5
  for (i in 1:length(V(ntwk))){
    if (distances(ntwk,CentralNode,i) <= NeighborhoodDistance){
      V(ntwk)$MyAttribute[i] <- 5
    }
  }
  return (ntwk)
...
newnet <- SpatiallyCorrelatedLabels3(newnet,FocalNode,2)
plot(newnet, vertex.color=ifelse(V(newnet)==FocalNode,"blue","red"), vertex.label.dist=2.5,edge.arrow.size=0.3,vertex.label.cex=ifelse(V(newnet)==FocalNode,1,0.4),vertex.size=V(newnet)$MyAttribute,main="Subgraph showing attribute values")


  
# Do a permutation test to see if the results when testing usaing Morans I look significant?
...
plot(net2, edge.arrow.size=.3,   vertex.size=5+2*V(net2)$MyAttribute,main="original attributes",vertex.label.cex=0.2)
...
# permute the network and re-evaluate
i <- FocalNode
V(net2)$PermedAttribute <- FixedPerm(V(net2)$MyAttribute,FocalNode)
cat("\nPermuted Attributes: ")
...
plot(net2, edge.arrow.size=.5,  vertex.label.cex=0.2, vertex.size=5+2*V(net2)$PermedAttribute,main="attributes of permuted network")
...
ff <- function(j){
    V(net2)$MyAttribute <- FixedPerm(V(net2)$MyAttribute,FocalNode)
    LStat[j] <- OldLisaNetForNodeI(net2,FocalNode)
}

#for (index in 1:SizeOfOurNetwork)
for (i in FocalNode:FocalNode)
{
  #cat("\n",index)
  # Evaluate Lisa stat for node i
  LStat=mclapply(1:NPerms, ff)
  LStat=unlist(LStat)
  PValue[FocalNode] <- sum(LStat>V(net2)$LISAstat[FocalNode])/NPerms
}
#PValue[FocalNode] <- sum(LStat>V(LISAnet)$LISAstat[FocalNode])/NPerms
(PValue)

boxplot(LStat)
abline(h=LStat[FocalNode],col="red",main="Distribution of LISA Stat")
